...
- GitHub Container Registry deploy workflow: image push에는 `packages: write`, provenance에는 `attestations: write`와 `id-token: write`를 둔다.
- Docker/Spring Boot container image: application jar를 먼저 빌드하고, layered jar extraction으로 runtime image를 구성한다.
...
| `push CD` | develop/main push | test 통과 후 image build/push/deploy | PR branch에서 production deploy |
...
CD까지 포함할 때는 아래를 추가한다.

1. 배포 대상 branch와 environment를 먼저 정한다.
2. image registry, image naming, rollback 기준을 문서화한다.
3. deployment secret 이름과 저장 위치를 문서화한다.
4. deploy job은 test와 image build job을 `needs`로 기다린다.
5. 필수 배포 secret이 없으면 deploy job을 실패시킨다. 배포를 skip하면서 성공처럼 보고하지 않는다.
6. 첫 배포 실패는 트러블슈팅 문서로 남긴다.
...
- CD workflow는 PR CI와 분리해도 된다.
- Trigger는 `push` to `develop/main`과 `workflow_dispatch`를 기본으로 한다.
- `develop`은 `staging`, `main`은 `production`으로 매핑한다.
- GitHub Environment를 사용해 staging/production secret을 분리한다.
- Docker image는 mutable tag와 immutable digest를 함께 남긴다.
- 배포에는 immutable digest를 우선 사용한다.
- `GITHUB_TOKEN`으로 registry push를 할 때 job permission에 `packages: write`를 둔다.
- Artifact attestation을 사용하면 `attestations: write`, `id-token: write`를 추가한다.
- 원격 서버 배포 secret은 GitHub Environment secret으로만 읽는다.
- workflow 파일에 host, key, token, password, webhook URL을 직접 쓰지 않는다.
- 원격 배포가 실제로 수행되지 않았는데 workflow가 성공하지 않게 한다.
- rollback은 이전 successful run의 image digest로 수행할 수 있게 문서화한다.

## Backend Container Deployment Shape
...
Spring Boot backend는 Gradle build cache를 살리기 위해 runner에서 `bootJar`를 먼저 실행하고, Dockerfile은 이미 생성된 jar를 layer extraction하는 방식이 기본이다.
...
Runtime image는 non-root user로 실행하고 `/actuator/health` 기반 health check를 둔다.
