...
이 문서는 워크스페이스 공통 브랜치 생성 및 관리 규칙을 정의한다.
...
- 루트 워크스페이스 저장소도 같은 GitFlow 전략을 따른다.
- 실제 프로젝트 저장소도 가능하면 같은 전략을 따르되, 프로젝트 고유 규칙이 있으면 그 규칙을 우선한다.
- 프로젝트 컨테이너와 실제 저장소 브랜치를 혼동하지 않는다.
